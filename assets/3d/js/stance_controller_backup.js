window.stanceController = {
    ...
    update: function (time) {
        // SELF-HEALING: If poses or bones are missing, try to initialize again
        if (!this.poses || Object.keys(this.poses).length === 0) {
            if (window.STANCE_DEFINITIONS) this.poses = window.STANCE_DEFINITIONS;
        }

        if (Object.keys(this.bones).length === 0) {
            // Try to init if model is ready
            if (window.model && window.findBone) {
                console.log("⚠️ StanceController: Late Initialization triggered in update loop");
                this.init();
            }
        }
