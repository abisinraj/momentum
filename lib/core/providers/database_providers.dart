...
@riverpod
Future<bool> isSetupComplete(IsSetupCompleteRef ref) async {
...
@riverpod
Stream<List<Workout>> workoutsStream(WorkoutsStreamRef ref) {
  final db = ref.watch(appDatabaseProvider);
...
@riverpod
Future<List<int>> todayCompletedWorkoutIds(TodayCompletedWorkoutIdsRef ref) async {
  final db = ref.watch(appDatabaseProvider);
...
@riverpod
Future<Workout?> nextWorkout(NextWorkoutRef ref) async {
  final db = ref.watch(appDatabaseProvider);
  try {
...
@riverpod
Future<Map<DateTime, String>> activityGrid(ActivityGridRef ref, int days) async {
  final db = ref.watch(appDatabaseProvider);
...
@riverpod
Future<List<Exercise>> exercisesForWorkout(ExercisesForWorkoutRef ref, int workoutId) async {
  final db = ref.watch(appDatabaseProvider);
...
@riverpod
Future<Map<String, int>> weeklyStats(WeeklyStatsRef ref) async {
  final db = ref.watch(appDatabaseProvider);
...
@riverpod
Future<String> weeklyInsight(WeeklyInsightRef ref) async {
  final db = ref.watch(appDatabaseProvider);
...
@riverpod
Future<List<Map<String, dynamic>>> sessionHistory(SessionHistoryRef ref, int limit) async {
  final db = ref.watch(appDatabaseProvider);
...
@riverpod
Future<List<Map<String, dynamic>>> sessionExerciseDetails(SessionExerciseDetailsRef ref, int sessionId) async {
  final db = ref.watch(appDatabaseProvider);
