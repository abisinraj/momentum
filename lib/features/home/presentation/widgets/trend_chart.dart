      ...
      child: AspectRatio(
        ...
        child: historyAsync.when(
          data: (sessions) {
            ...
            // Map to spots (X: Index, Y: Volume)
            // Note: Session object needs 'totalVolume'. If not present, we might need to calc 
            // or use duration as a proxy if volume isn't pre-calculated.
            // Assuming we might not have 'volume' readily available in the Session object list if it's lite.
            // Let's check session object... it's `Session`.
            // Ideally we need `SessionWithStats`.
            // For now, let's use Duration as a proxy OR assuming we can get volume.
            // Actually, `activeWorkoutSessionProvider` calculates volume. 
            // Let's assume for this MVP chart we graph Duration (easy) or Mock Volume if needed.
            // Better: Graph "Consistency" (1.0 for done) or Duration.
            // Let's graph Duration in Minutes for now as a real metric.
            
            List<FlSpot> spots = [];
            for (int i = 0; i < recent.length; i++) {
               final s = recent[i];
               final durationMins = s.durationSeconds / 60.0;
